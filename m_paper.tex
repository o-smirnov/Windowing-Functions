...
\begin{itemize}
 \item Fig.\ref{timessear1} shows that more than $99\%$ of the source brightness is measured for integration less than or equal to $25s$ 
...
$[0^\circ,0.5^\circ]$ with these shorts integrations.
  \item Fig.\ref{timessear2} also shows that more than $99\%$ of the source brightness is measured for channels less than or equal to 
...
out field of view sources (see Fig.\ref{timessear1} and Fig.\ref{timessear2}) and produced $\mathcal{N}_t\times\mathcal{N}_{\nu}$ uv-bins 
...
of the 
optimal image plane response (see Fig.\ref{optimalres.pdf}). 
\begin{figure*}
  \centering
...
$(4N_v)\times (4N_{pix})$ designed from $\mathbf{C}_{(t,\nu)}^{block,n_{block}}\cdot \mathbf{W}_{pq,(t,\nu)}^{block,n_{block}}\cdot 
...
$\mathbf{C}_{(t,\nu)}^{block,n_{block}}\cdot \mathbf{W}_{pq,(t,\nu)}^{block,n_{block}}\Big|_{p=0,\cdots,n_{a}-1}^{q=p+1,\cdots,n_{a}-1}$ on 
...
$\mathbf{V}_{pq,(t,\nu)}^{samp,n_{block}}\Big|_{p=0,\cdots,n_{a}-1}^{q=p+1,\cdots,n_{a}-1}=\sigma_{}\mathbf{G}$, 
...
n_{\nu}) \times (4 n_t n_{\nu})$ unity matrix. Eq.\ref{eq:noise1} can therefore be rewritten as:
\begin{eqnarray}
...
distorted on longer baselines compared to shorter ones. Therefore, if one have to attribute a $(u,v)$ weight, it may be proportional to 
...
describe the approach that we used with a WF to assign a proper weight to a uv-bin considering the \textit{spacing}\footnote{The 
\textit{distance} also has huge 
dependences on the baseline length and allows us to formally define the data
weight of a $uv$ point over the entire $uv$ plane.} between the baseline uv-bins and the centre uv-bin.\\

Fig.\ref{fig:uvcov} shows a snapshot coverage of an integration time interval. For shorter baselines, the tracks are closer to the centre 
of 
rotation and for longer baselines the tracks are farther away from this centre. The \textit{dot marks} are the data for a sampled uv-bin, 
and the  arrows indicates the separation between uv-bins and the centre uv-bin. It is trivial to see on this 
figure that these separations are wider on longer baselines. The results of boxcar averaging or BDWF averaging is assigned to the centre 
...
\end{definition}
The previous approach is exact and correct. But unfortunately, we do not suppressed confusion sources and we lost in 
sensitivity. Although boxcar averaging \textit{means high sensitivity}, we do need to eliminate confusion sources in such a 
way that the overall SNR becomes higher than that of boxcar averaging. We described such a methods in the following section.
...
of the WF ideal spectrum (image plane response magnitude), one need a wide cut-off interval as far as the spectrum approaches 
the ideal when the WF order increases. An overlap BDWF aims to extend the order of the BDWF in 
such a way that, we approach the ideal spectrum. The only drawback of this technique is the increase in time needed for processing the 
output sample of the signals being integrate.

Fig.\ref{fig:corrSigVLAMxBl_overlapGdelta} shows the behaviour of an overlap BDFW. We noticed from this figure that, with an overlap BDWF, 
the weight of a uv-bin is not defined by a unique BDWF, but by the strength of the correlation between the overall  overlapping BDWF on the 
bin. That said, the weight of a uv-bin becomes the summation of overlapping BDWF samples on the uv-bin normalized by the summation of the 
overall uv-bins weights within the range where these BDWF overlap are defined.
\begin{figure*}
...
Lobe width (MLW) and the Sidelobes Roll-off (SLR) for some WFs. To allow this paper to be useful both in signal processing and radio 
astronomy community, we presented in the below table the terminology that is used in radio astronomy in contrast of the terminology used 
...
\begin{tabular}{*3{c}}\\
 ...
 \hspace{-1cm}\begin{tabular}{|l|l|l|l|}
  ...
  {\footnotesize Spectral response or freq response} & {\footnotesize Image plane response (IPR)}\\
...
\begin{itemize}
  \item Conserved the signal within a field of interest "\textit{regime 1}": a WF with a narrower MLW.
  \item Achieved less masking of nearby sources by restricting "\textit{regime 2}" to a narrower band: a WF with a lower PSL.
  \item Suppressed sidelobes confusion from far away sources "\textit{regime 3}": a WF with a SLR that drops faster.
\end{itemize}
Such a WF is very common in theory, is a continuous 
function (unsampled function) over an illimitable interval, with unit area shown in Fig.\ref{fig:idealWF} and its ideal IPR in 
Fig.\ref{fig:idealIRF}. 
\begin{figure*}
\begin{minipage}{0.4\linewidth}\includegraphics[width=1.1\textwidth]{./Figures/idealsinc.png}\caption{Ideal 
WF}\label{fig:idealWF}
\end{minipage}
  \hspace{1cm} 
\begin{minipage}{0.38\linewidth}\includegraphics[width=1.\textwidth]{./Figures/idealIPR.png}\caption{Ideal 
...
\end{array}\right.
\end{equation}
Fig.\ref{fig:fig_box} and Fig.\ref{fig:fig_box_freq} give the graph of $\Pi(t)$ and its IPR  respectively. The blue and 
the red curves of Fig.\ref{fig:fig_box_freq} are the IPRs of $\Pi(t)$ for a cut-off time range $[-t_a, t_a]$ and 
$[-t_a/2,t_a/2]$ respectively. Note that when the cut-off range is large, the MLW of the IPR is 
...
Fig.\ref{fig:fig_gauss} and Fig.\ref{fig:fig_gauss_freq} give the graph of $G(t)$ and its IPR respectively, where $G(t)$ 
is truncated within the cut-off time range $[-t_a,t_a]$, with $b = 3$ for the blue curve and $b=5$ for the red curve. Note 
...
characterized by two independent parameters, the cut-off time range $[-t_a,t_a]$ and the order $p$. These two parameters control the 
FoV and sidelobes  attenuation. The time response of the Butterworth window is given by:
\begin{equation}
BW(t)= \Big(1 + (t/t_a)^{2p}\Big)^{-1}.
\end{equation}
For the same time range $[-t_a,t_a]$ we plotted  three curves $\{p=1, p=3, p=5\}$ of $BW(t)$ in Fig.\ref{fig:fig_butter} and 
their corresponding IPRs in Fig.\ref{fig:fig_butter_freq}. Note that when the order $p$ increases, the MLW of the 
IPR is conserved, while the PSL also increase and the SLR drops faster.
\subsection{Sinc Window}
The sinc window is defined as follow:
\begin{equation}
S(t)= sinc\big(\pi b t\big).
\end{equation}
 Fig.\ref{fig:fig_sinc} and Fig.\ref{fig:fig_sinc_freq} gives the graph of $S(t)$ and its IPR respectively, where 
$S(t)$ is truncated within the time range $[-t_a,t_a]$. Note that when the time range is large 
(Fig.\ref{fig:fig_sinc_freq}, blue curve), the IPR becomes perfectly flat at the pass band (FoV) while the MLW becomes narrower,  the PSL 
becomes lower and the SLR drops faster compared to a time range of $[-t_a/2,t_a/2]$ (Fig.\ref{fig:fig_sinc_freq}, red curve). 
...
\end{equation}
 Fig.\ref{fig:bessel} and Fig.\ref{fig:freq_resp_bessel} gives the graph of J$_0(t)$ and its IPR respectively, where 
J$_0(t)$ is truncated within the time range $[-t_a,t_a]$. Note that when the time range is large 
(Fig.\ref{fig:freq_resp_bessel}, blue curve), the IPR is flat at the pass band (FoV) while the MLW becomes narrower,  the PSL 
becomes lower and the SLR drops faster compared to a time range of $[-t_a/2,t_a/2]$ (Fig.\ref{fig:freq_resp_bessel}, red curve).
\subsection{The convolution theorem and WFs comparison}
...
measurement (see section \ref{sec:AvgCon}). When we limit the signal to the range $[-t_a,t_a]$ and convolved with a window $W_{t}$, it 
follows from the convolution theorem that this process was equivalent to "\textit{tapering the  Fourier transform of the signal}" by 
...
windows taken under consideration.\\
\begin{tabular}{*3{c}}
 \multicolumn{3}{|c|}{}\\
 \hspace{-1cm}\begin{tabular}{|l|l|l|l|}
...
SLR (db/octave) }  \\
  \hline\hline
  {\footnotesize $|\mathcal{F}\Big\{\Pi(t)\Big\}|=|S(t)|$} &{\footnotesize $\approx 0,073$} &{\footnotesize $-6,68$}&{\footnotesize 
$-6,78$}\\
  {\footnotesize $|\mathcal{F}\Big\{S(t)\Big\}|=|\Pi(t)|$} &{\footnotesize  $\approx0,306$}&{\footnotesize  $-11,22$}&{\footnotesize  
$-12,42$} \\
  {\footnotesize $|\mathcal{F}\Big\{G(t)\Big\}|$} & {\footnotesize $\approx0,0736$}&{\footnotesize  $-30,28$}&{\footnotesize  $-14,5$}\\ 
  {\footnotesize $|\mathcal{F}\Big\{BW(t)\Big\}|$} &{\footnotesize  $\approx0,079$} &{\footnotesize $-10,08$ }&{\footnotesize  $-15,39$}\\
  {\footnotesize $|\mathcal{F}\Big\{J_0(t)\Big\}|$} &{\footnotesize  $\approx 0,39$} &{\footnotesize $ -15,01$ }&{\footnotesize  $ -14,01$}
  \end{tabular}& \label{BDWBnoise}
\end{tabular}\\
\\

\hspace{-0.6cm}The sinc, Bessel of the first kind of order zero (J$_0$) and the Butterwordth (BW)  approach our specifications and they 
are taken under consideration in the rest of this paper.  We considered $sinc(X,Y)=sinc(X)sinc(Y)$, J$_0(X,Y)=$J$_0(\sqrt{X^2 + Y^2})$ and 
$BW(X,Y)=BW(\sqrt{X^2 + Y^2})$ for a two dimensional 
sinc, J$_0$ and BW respectively.
The following designations will be used in the rest of this paper:

Bl-WF-$n_{ovlpt}\times -$, Baseline dependent WF applied across the time direction, WF is one of the window under 
consideration (sinc, J$_0$ and BW) and $2n_{ovlpt}$ is the number of overlap time bins.

Bl-WF-$-\times n_{ovlp\nu}$, Baseline dependent WF applied across the frequency direction with $2n_{ovlp\nu}$ the number of overlap 
frequency bins.

Bl-WF-$n_{ovlpt}\times n_{ovlp\nu}$, Baseline dependent WF applied across both time and frequency direction (two dimensional BDWF).
\begin{figure*}
  \centering
...
\label{subsec:noise}
 We show in this section that on shorter baselines, the 
WFs under consideration reduce to the Boxcar window. Therefore, we effectively applying a hybrid filter; a Boxcar on the shorter baselines 
and 
the WF under consideration on the longer baselines (see Fig.\ref{fig:longshortmid-sinc}, Fig.\ref{fig:longshortmid-bessel} and 
Fig.\ref{fig:longshortmid-butter}). We furthermore evaluated the array
theoretical noise ratio, $R_{\epsilon}=\epsilon_{oo}^{bd}/\epsilon_{oo}^{bx}$ of BDWF 
averaging  by the one of boxcar averaging both predicted in Eq.\ref{eq:noise} and compare this ratio to the simulation one. For the 
analysis we used a JVLAC measurement set (MS) of 7min30s synthesis with a 1.5s
integration time at 1.4Ghz, with 150 channels of width 125kHz. We filled this MS with 1Jy thermal noise. We then processed boxcar 
...
the first overlap over 2min60s across time and 3125kHz across frequency and the second overlap over 3min45s across time and 6250kHz across 
frequency.
\begin{figure*}
 \centering
...
noise ($\epsilon^{bd}_{pq,oo}$) of BDWF 
averaging  by the one of boxcar averaging ($\epsilon^{bx}_{pq,oo}$). The ratio is plotted as a function of 
$\overline{\mathbf{d}}_t$ (the mean of the baseline $(u,v)$ distance). These figures shows that, the noise increases with baseline 
length.
   \item On shorter baselines, the noise estimate of BDWF averaging is approximately the same with the one 
of boxcar averaging. However, on shorter baselines, the noise drops significantly with the number of  overlap time and frequency bins. As 
...
2n_{ovlp\nu})\big)}$%\bigg)^{\frac{1}{2}}
\end{itemize}
The tables below summarized the array theoretical rms noise ratio and the simulation one.\\
...
In order to test the approaches described in section \ref{baseline1} and section \ref{baseline2}, we performed  multiple tests on JVLAC 
simulated MS. In 
this section we summarized and discussed those results. Two MS are used in 
our simulation, a high resolution MS (HR-MS) that contains the observed JVLAC data of short integration time and frequency, a low 
resolution 
...
created 
perfect clean maps. Each of the sky model is simulated 
...
averaging the results are saved into a JVLAC LR-MS of one timeslot of with  150s,  with 1 channels of width 6.25MHz. We 
...
and $n_{ovlp\nu}\in\{0,25,50\}$.
In Fig.\ref{fig:Bl-sinc-FoV2} to Fig.\ref{fig:Bl-bessel-FoV4} we
%Fig.\ref{fig:Bl-sinc-FoV4}, Fig.\ref{fig:Bl-bessel-FoV2} and
show the recovered source brightness as a function of the source radius centered at the phase centre.
% 
...
\begin{itemize}
 \item We recover more than $90\%$ of the source flux density within $\approx 75\%$ of \textit{regime 1} with $Bl$-$sinc$-$0 \times 0$, 
      $Bl$-J$_0$-$0\times0$ and $Bl$-BW-$0\times0$. These are good results compare to boxcar averaging.
 \item We recover more than $90\%$ of the source flux density within $\approx 95\%$  of \textit{regime 1} with the overlap sinc and 
J$_0$\footnote{As 
...
the window theoretical representation}. This is not the case with the 
overlap Butterwordth because in \textit{regime 1}, the IPR of the Butterwordth is similar to the IPR of the boxcar (see 
Fig.\ref{fig:fig_butter_freq}). 
\item  In \textit{regime 2}, it is seen on these Figures that the baseline dependent J$_0$ performed well compared to the baseline 
dependent sinc as the curve of the baseline dependent J$_0$ drops faster. The result of the baseline dependent BW is not accurate in this 
\textit{regime 2}. Nevertheless, one can manipulate the order $p$ of BW in the aims to achieve an accurate result.
...
This research has made use of the MeqTrees software system, designed to implement numerical models for third-generation calibration (3GC), 
 Python Extensions for Interferometry and Python. It has been supported by the South Africa National Research Foundation.
