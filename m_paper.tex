...
$\Delta t$ and  $\Delta \nu$ are large enough to save memory, this work investigates various windowing functions (WF) in term of the 
...
\mathbf{S}_{pq,(t,\nu)}^{n_{block}}\mathbf{F}\cdot\mathcal{I}_{l,m}^{sky },\label{eqv:linear}
\end{equation}
...
\end{equation}
Our data is always corrupted by a random error component or noise, $\epsilon$. $\mathbf{A}$ is a \textit{design} matrix of size 
...
without a source, $\mathbf{V}_{pq,(t,\nu)}^{samp,n_{block}}\Big|_{p=0,\cdots,n_{a}-1}^{q=p+1,\cdots,n_{a}-1}=\sigma_{uv}^i|_{i=1\cdots 
N_v\times n_t \times n_{\nu}}$, where $\sigma_{uv}^i=\sigma_{uv}^j$ ($i\neq j$) is the expected rms noise per visibility before boxcar 
averaging or BDWF averaging and $N_v \times n_t \times n_{\nu}$ is the total number of these visibilities. For convenience let  
$\sigma_{uv}^i=\sigma_{uv}$, Eq.\ref{eq:noise1} can therefore be rewritten as:
\begin{eqnarray}
 \widetilde{\epsilon}_{o,o}		
&=&\frac{\sigma_{uv}}{N_{v}}\bigg(1-\Big(\mathbf{F}^{H}\cdot\mathbf{A}\Big)_{(o,o)}\bigg)\bigg(\mathbf{B}^{\dagger}\cdot\mathbf{G}
\bigg).\label{eq:noise}
\end{eqnarray}
Here, $\mathbf{G}$ is a $(N_v 4 n_t n_{\nu}) \times (4 n_t n_{\nu})$ unity matrix.
...
Lobe width (MLW) and the Sidelobes Roll-off (SLR) for some WFs. 
The terminology that will be using in this paper in contrast of the terminology used in signal processing is summarized in the table 
below.
\begin{tabular}{*3{c}}\\
 \multicolumn{3}{|c|}{}
...
The goal of this study is twofold, find a WF that its IPR will conserved the signal within a field of interest while 
suppressing sidelobes confusion from strong sources outside this field of interest. Such a WF is very common in theory, is a continuous 
function (unsampled function) over an illimitable interval with unit area (see Fig.\ref{fig:idealWF}). The ideal IPR of this WF is as 
presented in Fig.\ref{fig:idealIRF}. 
\begin{figure*}
\begin{minipage}{0.38\linewidth}\includegraphics[width=1.\textwidth]{./Figures/idealsinc.png}\caption{Ideal 
WF}\label{fig:idealWF}
\end{minipage}
  \hspace{1cm} 
\begin{minipage}{0.45\linewidth}\includegraphics[width=1.2\textwidth]{./Figures/boxideal.png}\caption{Ideal 
...
\end{equation}
One can replaced the convolution window by its Fourier transform. Therefore, we want a convolution window 
($W_{t}$ or $\mathcal{F}\Big\{W_{t}\Big\}$) with a narrower MLW (for a wide dynamic range or to maintain the desire FoV), lowers PSL (to 
achieve less masking of nearby sources), and faster SLR (to achieve less masking for far away sources). 
 The table below summarizes the MLW, the PSL and the SLR of $W_{t}$ or $|\mathcal{F}\Big\{W_{t}\Big\}|$ under consideration.\\
\begin{tabular}{*3{c}}
 \multicolumn{3}{|c|}{}\\
 \hspace{-1cm}\begin{tabular}{|l|l|l|l|}
...
\hspace{-0.6cm}In the rest of this paper we only consider the sinc 
window, the Bessel of the first kind and the Butterwordth as they approach our specifications (see the table above).  We considered 
$sinc(X,Y)=sinc(X)sinc(Y)$, J$_0(X,Y)=$J$_0(\sqrt{X^2 + Y^2})$ and $BW(X,Y)=BW(\sqrt{X^2 + Y^2})$ for a two dimensional 
sinc window, Bessel of the first kind and Butterwordth window respectively.
\begin{figure*}
  \centering
...
Fig.\ref{fig:longshortmid-butter}). We furthermore evaluated the 
theoretical noise predicted in Eq.\ref{eq:noise} (see \ref{fig:per-baseline-noise-ratio-sinc}, 
Fig.\ref{fig:per-baseline-noise-ratio-bessel} and \ref{fig:per-baseline-noise-ratio-BW}).
\begin{figure*}
 \centering
