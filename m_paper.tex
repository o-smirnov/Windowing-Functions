...
\end{equation}
where, $b=(2\sigma_1^2)^{-1}$. The inverse Fourier transform of Eq.\ref{eq:gauss} is given by 
$\mathcal{F}^{-1}\big\{G(t)\big\}=\sqrt{\frac{b}{\pi}}e^{-cl^2}$, where $c=\pi^2/b$.
This shows us that the inverse Fourier transform of a Gaussian with standard deviation $\sigma_1$ is a Gaussian with a standard 
...
consideration (sinc, J$_0$ and BW) and $2n_{ovlpt}$ is the number of overlap time bins (one dimensional BDWF).

Bl-WF-$-\times n_{ovlp\nu}$: Baseline dependent WF applied across the frequency direction with $2n_{ovlp\nu}$ the number of overlap 
frequency bins (one dimensional BDWF).

Bl-WF-$n_{ovlpt}\times n_{ovlp\nu}$: Baseline dependent WF applied across both time and frequency direction (two dimensional BDWF).

Bx-avg-$n_{ovlpt}\times -$: Boxcar averaging applied across the time direction (one dimensional boxcar averaging).

Bx-avg-$-\times n_{ovlp\nu}$: Boxcar averaging applied across the frequency direction (one dimensional boxcar averaging). 

Bx-avg-$n_{ovlpt}\times n_{ovlp\nu}$: Boxcar averaging applied across both time and frequency direction (two dimensional boxcar averaging).
\begin{figure*}
  \centering
...
averaging  by the one of boxcar averaging both predicted in Eq.\ref{eq:noise} and compared this ratio to the simulation one. For the 
...
$0^\circ$ to $1^\circ$; "\textit{Regime 2}" from $1^\circ$ to $2^\circ$ and "\textit{Regime 3}" from $2^\circ$ to infinity.
...
$0^\circ$ 
to $2^\circ$; "\textit{Regime 2}" from $2^\circ$ to $4^\circ$ and "\textit{Regime 3}" from $4^\circ$ to infinity. 
% 
...
the curves of boxcar averaging (see bx-avg) mid that of Bl-sinc-$0\times0$ and Bl-J$_0$-$0\times0$ which implied an equivalent flux density 
attenuation while the curve of 
Bl-sinc-$100\times25$, Bl-sinc-$150\times50$, Bl-J$_0$-$100\times25$ and Bl-J$_0$-$150\times50$ are below the one of bx-avg which implied 
...
bins increase, the attenuation becomes significant. The similar results occurs with BW BDWF averaging $1^{\circ}$ away from the regime 
starting interval.
\end{itemize}
...
to J$_0$ while J$_0$ performed well in \textit{Regime 2} and \textit{Regime 3} compared to the sinc.
The great potential  of the overlap windows is the accurate performance in the three regimes. An interesting improvement of this study is 
the possibility of using WF capable of providing higher dynamic range images and attenuating sidelobes confusion. This advanced techniques 
is required by the radio telescopesof the future.
% 
% suppression
...
coordinates with radius $r\in\{0.09^\circ,0.25^\circ,0.5^\circ,1^\circ,1.5^\circ, 2^\circ\}$.
\subsubsection{Frequency direction}
Boxcar and BDWF averaging are processed across the frequency direction.
Each of the sky models is simulated with a JVLA HR-MS of 1 timeslot of width $\Delta t^{hrms}=0.1s$ at $1.4$GHz. The raison of a short 
HR-MS 
