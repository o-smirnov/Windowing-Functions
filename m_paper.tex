...
respectively (see appendix \ref{app:complexmatrices} for the derivation of these complex matrices). The formalism of Eq.\ref{eq:noise} will 
be used to derive the noise estimate in section \ref{subsec:noise}.
...
overall uv-bins weights within the range where these BDWF overlap.
\begin{figure*}
...
($W_{t}$ or $\mathcal{F}\Big\{W_{t}\Big\}$) with a narrower MLW (for a better spectral resolution), lowers PSL (to 
achieve less masking of nearby sources), and faster SLR (to achieve less masking for far away sources). 
 The table below summarizes the MLW, the PSL and the SLR of $|\mathcal{F}\Big\{W_{t}\Big\}|$ under consideration.\\
\begin{tabular}{*3{c}}
 \multicolumn{3}{|c|}{}\\
...
SLR (db/octave) }  \\
  \hline\hline
  {\footnotesize $|\mathcal{F}\Big\{\Pi(t)\Big\}|$} &{\footnotesize $\approx 0,073$} &{\footnotesize $-6,68$}&{\footnotesize $-6,78$}\\
  {\footnotesize $|\mathcal{F}\Big\{S(t)\Big\}|$} &{\footnotesize  $\approx0,306$}&{\footnotesize  $-11,22$}&{\footnotesize  $-12,42$} \\
