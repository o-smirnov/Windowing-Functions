...
samples, rather than a continuous integration. To represent this properly, we need to replace the integral of 
eqs.~(\ref{eq2:conti}) by a discrete sum:
\begin{equation}
\label{eq:discrete:tf0}
V^\mathrm{(m)}_{pqkl} = \sum_{ij\in\Bij_{kl}}  \VV_{pq}(t_i,\nu_j),
\end{equation}
where $\Bij_{kl}$ is the set of sample indices $ij$ corresponding to the sampling bin, i.e.
\begin{equation}
\Bij_{kl} = \big \{ ij:~t_i\nu_j \in \Btf_{kl} \big \}.
\end{equation}
Using the BDWF definitions above, this becomes a conventional discrete convolution (assuming a regular $t\nu$ grid):
\begin{equation}
...
V^\mathrm{(m)}_{pqkl} = \sum_{i,j=-\infty}^{\infty}  \VV_{pq}(\bmath{u}_{ij}) \Puv_{pqkl}(\bmath{u}_{ij}-\bmath{u}_{kl}),
\end{equation}
...
for computing the averaged visibility sample $kl$:
\begin{equation}
\label{eq:avg:bessel}
V^\mathrm{(m)}_{pqkl} = \frac{\displaystyle\sum\limits_{{i,j}\in B_{kl}} \VV_{pq}(\bmath{u}_{ij}) J_0(\bmath{u}_{ij}-\bmath{u}_{kl})}
{\displaystyle\sum\limits_{{i,j}\in B_{kl}} J_0(\bmath{u}_{ij}-\bmath{u}_{kl})},
\end{equation}
where $\bmath{u}_{kl}$ is the midpoint of the sampling bin in $uv$-space. The main lobe of the WF then has the same scale
across the entire $uv$-plane, while the sampling bins have different sizes. Conversely, in $t\nu$-space the sampling
bins are regular, while the main lobe of the effective WF scales inversely with the baseline fringe rate. Furthermore, 
the WF is truncated at the edge of each sampling bin; on the shortest baselines this truncation is extreme to the 
...
A more sophisticated approach involves overlapping BDWFs. Normal averaging implicitly assumes that the bins $B_{kl}$ 
...
sampling bin: while the latter represents the spacing of the samples in the output, the former represents the size 
...
sidelobes -- and decreased noise, as more samples are taken into account.

\begin{figure} 
\includegraphics[width=\columnwidth]{./Figures/corrSigVLAMxBl_overlapGdelta.pdf}\caption{Overlapping 
BDWFs representing adjacent sampling bins. This corresponds to overlap factor $\alpha=3$ in time.}\label{fig:overlap}
\end{figure}
...
\subsection{Noise penalty estimates}
\label{sec:imaging}

Replacing an unweigLet us now work out analytically the \emph{noise penalty} associated 
