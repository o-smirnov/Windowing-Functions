...
resample the latter using a range of BDWFs, convert the visibilities to dirty images, and measure the peak source 
...
\begin{figure*}
\includegraphics[width=.49\textwidth]{./Figures/suppression-25-100-sincs.png}\hfill%
\includegraphics[width=.49\textwidth]{./Figures/relsuppression-25-100-sincs.png}\\
\caption{Left: smearing as a function of distance from phase centre, for 25s 2MHz and 100s 10MHz averaging, and for 
various BDWFs. The noise penalty $\Ksi$ and the far-source suppression factor $F$ are given relative to 25s 2MHz 
averaging. Right: same results normalized to the 25s 2MHz curve.}
\label{fig:results-example}
\end{figure*}

A typical performance comparison for JVLA-C 1.4 GHz is given by Fig.~\ref{fig:results-example}. This figure illustrates 
some of the principal achievements of the present work, so let us spend some time explaining it. The horizontal axis 
represents distance from phase centre, while the vertical axis represents the degree of smearing. Unity corresponds to no smearing;
this is the case at phase centre, thus all curves start at unity. The three thick grey curves correspond to normal
averaging into 25s$\times$2MHz, 50s$\times$4MHz, and 100s$\times$10MHz. We can (rather arbitrarily) define an ``acceptable'' 
level of smearing to be at most 5\% at the half-power point. The half-power beam radius of the JVLA at this frequency is 
$\approx=0.25^\circ$, this is indicated by the vertical line in the figure. The thin horizontal line is the
acceptable smearing threshold of $0.95$. We can readily see that averaging at 100s$\times$10MHz (the lower thick grey curve)
leads to smearing levels just below this threshold. Were the JVLA equipped with dishes one-quarter of its present size,
the primary FoV would be a factor of four larger, and averaging of 25s$\times$2MHz would be necessary.
