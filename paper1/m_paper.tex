...
while $u$ points East and $v$ North. Given a sky intensity distribution $I(l,m)$, where $l,m$ are the direction cosines,
...
the dependence on frequency is explicit. We can express this by treating $V_{pq}$ in eq.~(\ref{eq:visSky:2D}) as 
a function of $t,\nu$:
\begin{equation}
...
Taking this effect into account, Eq. \ref{eq2:visSky} is  rewritten as an integration over time and frequency interval. If $[t_s,t_e]$ and 
 $[\nu_s,\nu_e]$ are the time and frequency integration interval respectively, then Eq.\ref{eq2:visSky} can be re-written as:
% variates in 
...
\subsection{Effect of time and bandwidth boxcar averaging}
\label{sec:effectbw}
This problem arises when the interferometer array is used to observe wide fields. Finite time and frequency 
integration intervals make  Eq.\ref{eq2:conti} to hold and in the sampled form in the limit when $(\Delta t, \Delta 
\nu) \rightarrow (0,0)$. Nevertheless, as mention earlier, the Fourier phase components $2\pi \phi(u,v,w)$ is a function of the direction 
in the sky, the wavelength, the separation of 
...
centre) and courses the distortion of these sources.  The distortion becomes significant when $\sqrt{l^2 + m^2}$ increases and  for large 
time and wide frequency intervals. This is the raison of which the degree of smearing 
...
Figures \ref{timessear1} and \ref{timessear2} show the attenuation of  1 Jy source at various coordinates in the sky for various 
integration time intervals and channels respectively. The instrument used is the JVLAC configuration.
\begin{itemize}
...
out of the interval for shorts integrations.
  \item Figure \ref{timessear2} also shows $\approx 99\%$ of the source flux density measured for channels $\leq1250$kHz when the 
source is within $[0^\circ,0.5^\circ]$. The same phenomenon  in Figure \ref{timessear1} is observed when the source is 
...
and $F = N_{\nu}\times \Delta \nu$. Here, $N_t$ and  $N_{\nu}$ are the number of time sample and frequency channels entering the Fourier 
domain respectively. The reduction of smearing becomes proportional to the width of the time integration and the channel. 
...
\begin{figure*}
  \centering
%   distance.plot(wind1='',r1='Rav10.npy',l1='Lav10.npy',l2='Lav25.npy',l3='Lav50.npy',l4='Lav100.npy',l5='Lav150.npy',title=r'Time averaging; 
% JVLAC snapshot; natural weighting',xlabel=r'Distance from the phase centre[deg]')
...
\hspace{1cm}
% distance.plot(wind1='',r1='Rav10000.npy',l1='Lav100000.npy',l2='Lav1250000.npy',l3='Lav2500000.npy',l4='Lav5000000.npy',l5='Lav6250000.npy',
% title=r'Bandwidth averaging; JVLAC snapshot; natural weighting',xlabel=r'Distance from the phase centre[deg]')
...
\begin{minipage}{0.38\linewidth}\includegraphics[width=1\textwidth]{./Figures/sinc.png}\caption{Sinc 
window}\label{fig:fig_sinc}\end{minipage}
\hspace{1cm}
\begin{minipage}{0.38\linewidth}\includegraphics[width=1\textwidth]{./Figures/freq_resp_sinc.pdf}\caption{Frequency response of the 
sinc window }\label{fig:fig_sinc_freq}\end{minipage}\\
\begin{minipage}{0.38\linewidth}\includegraphics[width=1\textwidth]{./Figures/Butterwordth.pdf}\caption{ Butterwordth 
...
\begin{minipage}{0.38\linewidth}\includegraphics[width=1\textwidth]{./Figures/gausian.pdf}\caption{Gaussian windows}\label{fig:fig_gauss}
\end{minipage}
\hspace{1cm}
\begin{minipage}{0.38\linewidth}\includegraphics[width=1\textwidth]{./Figures/freq_resp_gaussian.pdf}\caption{Frequency response of 
