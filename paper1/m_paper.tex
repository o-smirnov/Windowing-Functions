...
Each visibility sample (whether at the correlator output, or after averaging in post-processing) corresponds to the 
...
\begin{equation}
V_{pq}^\mathrm{samp}(t_c,\nu_c) = \frac{1}{\Delta t \Delta \nu} 
\int\limits_{t_s}^{t_e}\int\limits_{\nu_s}^{\nu_e}V_{pq}(t,\nu)}d\nu dt.
\label{eq2:conti}
\end{equation}
Note that while  this ignores the complexities of correlator architecture (where the visibilities are channelized via 
