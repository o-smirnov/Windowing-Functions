...
\begin{equation}
\label{eq:imaging3}
\II^D =  \sum_{pqkl} W_{pqkl} \II^D_{pqkl},~~
\II^D_{pqkl} =  \FF^{-1}[ \VV^\mathrm{(m)}_{pqkl} ],
\end{equation}
which shows that the dirty image $\II^D$ is a weighted sum of dirty images corresponding to the individual visibility 
...
\end{equation}
where the right-hand side is given by eq.~(\ref{eq:visSky:2Dtf}). However, an interferometer necessarily measures the
...
eqs.~(\ref{eq2:conti}-\ref{eq2:conti:uv1}) by discrete sums, in which case the convolution operation becomes 
a discrete convolution, but the derivations do not change.
...
The discussion above provides an alternative way to look at smearing. Combining eqs.~(\ref{eq:imaging3}) and (\ref{eq:avscon}),
...
\begin{figure}
\includegraphics[width=\columnwidth]{./Figures/idealIPR.png}
\caption{Boxcar response. In the $uv$-plane, this represents the windowing function corresponding to normal
averaging of visibilities. In the image plane, this represents the ideal image-plane response function.}\label{fig:idealIRF}
\end{figure}

\begin{figure}
\includegraphics[width=\columnwidth]{./Figures/idealsinc.png}%
\caption{Sinc response. In the image plane, this represents the response function corresponding to a boxcar WF in 
