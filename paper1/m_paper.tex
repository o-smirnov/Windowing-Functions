...
This can approximated by subdividing the averaging interval into $n_t\times n_\nu$ samples, and doing a discrete average:
\begin{equation}
...
functio that selects points where we sample the visibilities: $V^\mathrm{samp}_{pq} = S_{pq}\cdot V_{pq}$. From this point on we will work with the sampled visibilities $V^\mathrm{samp}$ only.
