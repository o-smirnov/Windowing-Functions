...
% }
% \author[M. Atemkeng , O. Smirnov, C. Tasse, G. Foster and J. Jonas]{M. Atemkeng$^{1}$, O. Smirnov$^{12}$\thanks{E-mail: m.atemkeng@gmail.com}, 
...
%>>>>>>> cacce7cc6b2cb474c05287a4eab60064759d41bb
...
correlator can support, and is normally much smaller than the PB \citep[{\bf for an extensive discussion see}][]{keimpema2015sfxc}. Modern VLBI correlators overcome this by employing a technique 
...
\begin{equation}
\label{eq:imaging3}
\IID =  \sum_{pqkl} W_{pqkl} \FF^H\{ \VVM_{pqkl} \},
\end{equation}
...
\begin{equation}
\label{eq:imaging:DI}
\IIDI =  \sum_{pqkl} W_{pqkl} \PP_{pqkl} \circ \II,~~\PP_{pqkl}=\FF^H\{ \delta_{pqkl} \}
\end{equation}
...
improvements in calibration techniques \ATM{could not find this reference}\cite{Perley2013}, allow imaging to be done in the first PB sidelobe as well
...
\FilterFigure{./Figures/gaussiangrey.png}{./Figures/freq_resp_gaussiangrey.png}{Gaussian  and its tapering response.}{fig:wf:gauss}
...
\FilterFigure{./Figures/butterworthgrey.png}{./Figures/freq_resp_butterworthgrey.png}{Butterworth  and its tapering response.}{fig:wf:bw}
...
spheroidal wave (of oder zeros) and its tapering response.}{fig:wf:prolate}
...
\FilterFigure{./Figures/sincgrey.png}{./Figures/freq_resp_sincgrey.png}{Sinc and its tapering response.}{fig:wf:sinc}
...
\label{Bessel}
\FilterFigure{./Figures/besselgrey.png}{./Figures/freq_resp_besselgrey.png}{$J_0$ and its tapering response.}{fig:wf:bessel}
...
In the VLBI regime, it is usually a combination of smearing and data rates,
rather than the primary beam, that effectively limits the FoV. For example, the current European VLBI Network (EVN) correlator operated by the Joint Institute for VLBI in Europe (JIVE) is capable of producing data at a 
10ms dump rate with 16 MHz of total bandwidth split into 8192 channels. The maximum available FoV is restricted by the smallest primary beam, which is usually that of the 100m Effelsberg telescope -- about $10'$ at L-band. At the full time and frequency resolution of the correlator, smearing is acceptably low at $r=5'$, however, it is usually prohibitively expensive to store and process data as such high resolution. Techniques such as multiple-phase centre correlation \citet{Deller} have been used instead. More typically, the data is averaged into more modest sampling rates of 2s and 32 channels, which 
causes smearing to restrict the FoV to about ... (Paragi priv. comm.) This limits the scientific usefullness of the EVN archive.

In this section we investigate whether the use of BDWFs can enable true wide-field VLBI. We simulate a 1.6 GHz EVN observation employing eight stations 
(Effelsberg, Hartebeesthoek, Jodrell Bank, Noto, Onsala, Torun, JVLA, Westerbork, Shanghai), with a maximum baseline of 10161km. Figure~\ref{fig:results-vlbi} compares the smearing response of normal averaging to that of an
overlapping Bessel BDWF at 0.5s and 26.5. This represents a compression ratio of about 500 compared to full resolution data; the sensitivity loss is a factor of 2. 
...
In Fig.~\ref{fig:results-vlbi}, we investigate the performance of BDWFs in the regime of VLBI. Here, we simulate a 1.6 GHz VLBI observation employing eight stations 
...
primary beam of any particular station. In the past, it was shown possible to achieve a FoV corresponding to the entire
VLBI primary beam by providing sufficient spectral and time resolution. Unfortunately, this was directly 
forgotten because of the wide amount of data to be recorded \citep{lenc2008deep,chi2013deep}

This has led to the development of techniques such as multiple-phase centre
