...
interferometer and $c$ the speed of the light. The measurement in Equation (\ref{eq:1}) is over the entire 
...
\end{alignat}
Here, $f(\mathbf{s})$ is the image plane tapering funtion or response function.
...
given in wavelenght and $l,m, \sqrt{1-l^2-m^2}-1$ are the components of  $\mathbf{s}-\mathbf{s_0}$ given in radian, describing the 
three directions cosine. 
We can write:
\begin{alignat}{2}
V_{\nu}(\mathbf{b})  =& \iint\limits_{-\infty}^{\hspace{.3cm}+\infty}f_{lm}(u_{t\nu},v_{t\nu})e^{-2\pi i (u_{t\nu}l+v_{t\nu}m+w_{t\nu}(n-1))}dt d\nu\\
		      =& \check{f}_{lm}(u_{t\nu},v_{t\nu})\circ\iint\limits_{-\infty}^{\hspace{.3cm}+\infty}e^{-2\pi i (u_{t\nu}l+v_{t\nu}m+w_{t\nu}(n-1))}dt d\nu \label{eq:4}
...
segments across time and frequency, thus over a square taper. Eq.\ref{eq:4} becomes:
\begin{alignat}{2}
V_{\nu}(\mathbf{b}) =& \check{f}_{lm}(u_{t\nu},v_{t\nu})\circ \\
		  &\Bigg[\frac{1}{\Delta \nu \Delta t}\int_{\nu_c - \frac{\Delta \nu}{2}}^{\nu_c + \frac{\Delta \nu}{2}}\int_{t_c - \frac{\Delta t}{2}}^{t_c + \frac{\Delta t}{2}}
 e^{-2\pi i (u_{t\nu}l+v_{t\nu}m+w_{t\nu}(n-1))}dt d\nu\Bigg]\\
					 %=& \frac{1}{\Delta \nu \Delta t}\int_{\nu_c - \frac{\Delta \nu}{2}}^{\nu_c + \frac{\Delta \nu}{2}}\int_{t_c - \frac{\Delta t}{2}}^{t_c + \frac{\Delta t}{2}}
%e^{-2\pi i (u_{t\nu}l+v_{t\nu}m+w_{t\nu}(n-1))} dt d\nu\\
					 =&\check{f}_{lm}(u_{t\nu},v_{t\nu})\circ \\
					  &\Bigg[sinc\frac{-2\pi t\Delta \nu}{2}sinc\frac{-2\pi\nu\Delta t}{2}e^{-2\pi i (u_{t_c\nu_c}l+v_{t_c\nu_c}m+w_{t_c\nu_c}(n-1))}\Bigg]\label{eq:5}
\end{alignat}
...
O.M. Smirnov (cite, Jan 2011) demonstrated that in the case where $f_{lm}(u_{t\nu},v_{t\nu})$ is a square taper, the bove Equation is approximated in term of the
phase changes in time $\Delta \Psi$ and frequency  $\Delta \Phi$ for the case
of smearing as:
\begin{alignat}{2}
V_{\nu}(\mathbf{b})^{box}_{t_c \nu_c} \simeq &  sinc\frac{\Delta \Psi}{2}sinc\frac{\Delta \Phi}{2}e^{-2\pi i (u_{t_c\nu_c}l+v_{t_c\nu_c}m+w_{t_c\nu_c}(n-1))}\label{eq:6},
\end{alignat}
with,
\begin{alignat*}{2}
...
\begin{alignat}{2}
V_{\nu}(\mathbf{b})^{window}_{t_c \nu_c} &\simeq \check{f}_{lm}(u_{t_c\nu_c},v_{t_c\nu_c})\circ \\
					  & \Bigg[sinc\frac{\Delta \Psi}{2}sinc\frac{\Delta \Phi}{2}e^{-2\pi i (u_{t_c\nu_c}l+v_{t_c\nu_c}m+w_{t_c\nu_c}(n-1))}\Bigg]\label{eq:5}.
\end{alignat}
